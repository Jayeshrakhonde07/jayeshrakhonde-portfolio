  ...
  return (
    <section className='min-h-screen flex items-center  px-6 md:px-8 lg:px-10 py-20' id='about'>
      ...
      <div className='max-w-7xl mx-auto w-full '>
        {/* heading component  */}
        <SectionTitle title = {"About Me"} subtitle = {"Get To "} spantitle = {"Know Me"} />


        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mt-4'>

          {/*information about me */}
          <div className='bg-bg-card/70 border border-card-border p-4 md:p-6 rounded-md backdrop-blur-md hover:border-card-hover hover:shadow-card hover:-translate-y-1 transition duration-500'>
            <h2 className='font-bold mb-2 md:text-xl '>Building ideas into meaningful digital experiences</h2>
            <p className='text-text-body text-justify [hyphens:auto]  mb-4'>
              Hello! I’m <span className='text-text-main font-medium'>Jayesh Kishor Rakhonde</span>, an Information Technology student pursuing my <span className='text-accent font-medium'>B.Tech in Information Technology</span> at <span className='text-text-main font-medium'>Prof. Ram Meghe Institute of Technology and Research, Badnera.</span> I’m from Nandura, Maharashtra, and I’ve always been curious about how websites and applications work. I enjoy learning new technologies and turning my ideas into something that people can actually use.</p>
          ...
          <div className='flex flex-col gap-4'>
            ...
            <div className='flex flex-col gap-4'>
              {about.information.map((info)=>{
                ...
                return(
                  <div key={info.label} className='group bg-bg-card flex items-center gap-3 px-3 py-2 border border-card-border rounded-md hover:border-card-hover hover:shadow-card hover:-translate-y-1 transition duration-500'>

                    <div className='bg-badge p-3 rounded-md border text-accent border-card-border group-hover:bg-badge-hover group-hover:text-bright-accent  group-hover:border-card-hover   transition duration-300'>
                      <Icon aria-hidden="true" />
                    </div>
                    <div>
                      <h3 className='text-accent font-medium'>{info.label}</h3>
